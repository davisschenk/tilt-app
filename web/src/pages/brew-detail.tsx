              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  ...
                  {brew.status !== "Archived" && (
                    <DropdownMenuItem onClick={() => handleStatusChange("Archived")} disabled={updateBrew.isPending}>
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  <div>
                    ...
                    <p className="font-medium">{hydrometer?.color ?? brew.latestReading?.color ?? "Unknown"}</p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">Start Date</p>
                    <p className="font-medium">{brew.startDate ? format(new Date(brew.startDate), "MMM d, yyyy") : "—"}</p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">End Date</p>
                    <p className="font-medium">{brew.endDate ? format(new Date(brew.endDate), "MMM d, yyyy") : "—"}</p>
                  </div>
                </CardContent>
              </Card>
              {brew.targetFg != null && (
                <Card>
                  ...
                  <CardContent>
                    {analytics == null ? (
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <StatItem label="Predicted Date" value={format(new Date(analytics.predictedFgDate), "MMM d 'at' HH:mm")} />
                        <StatItem
              ...
              brew.latestReading.gravity <= brew.targetFg && (
                <div className="flex items-center gap-3 rounded-md border border-green-300 bg-green-50 p-4 mb-6 dark:border-green-800 dark:bg-green-950">
                  ...
                  <div className="flex-1">
                    <p className="font-medium text-green-800 dark:text-green-200">Target gravity reached!</p>
                    <p className="text-sm text-green-700 dark:text-green-300">Consider completing this brew.</p>
                  </div>
                  <Button size="sm" onClick={handleFinishBrew} disabled={updateBrew.isPending}>
                    <CheckCircle className="mr-2 h-4 w-4" />
                    Finish Brew
                  </Button>
                </div>
            ...
            <div className="space-y-3">
              {(() => {
                ...
                if (triggered.length === 0) {
                  return <p className="text-muted-foreground text-sm py-4">No alerts fired yet.</p>;
                }
